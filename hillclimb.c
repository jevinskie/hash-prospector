...
static void hash_gen(struct hash *h, uint64_t rng[4]) {
...
static int hash_equal(const struct hash *a, const struct hash *b) {
    for (int i = 0; i < HASHN; i++) {
        if (a->c[i] != b->c[i])
            return 0;
        if (a->s[i] != b->s[i])
...
static void hash_print(const struct hash *h) {
    putchar('[');
    for (int i = 0; i < HASHN; i++)
...
static int hash_parse(struct hash *h, char *str) {
    long s;
    unsigned long c;
    char *end, *tok;
    if (*str != '[')
...
static uint32_t hash(const struct hash *h, uint32_t x) {
    for (int i = 0; i < HASHN; i++) {
        x ^= x >> h->s[i];
        x *= h->c[i];
    }
...
static double estimate_bias32(const struct hash *f, uint64_t rng[4]) {
    long n            = 1L << QUALITY;
    long bins[32][32] = {{0}};
    for (long i = 0; i < n; i++) {
...
static double exact_bias32(const struct hash *f) {
...
static void hash_gen_strict(struct hash *h, uint64_t rng[4]) {
    do
        hash_gen(h, rng);
    while (estimate_bias32(h, rng) > THRESHOLD);
}

static uint64_t load64(const void *buf) {
...
static uint64_t hash64(uint64_t x, uint64_t m) {
    x *= m;
    x ^= x >> 32;
    return x;
}

static void mix64x4(uint64_t x[4]) {
...
static void rng_init(uint64_t rng[4]) {
...
static uint32_t modinv32(uint32_t x) {
    uint32_t a = x;
    x += x - a * x * x;
    x += x - a * x * x;
    x += x - a * x * x;
