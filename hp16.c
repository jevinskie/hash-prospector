...
static unsigned long long hash64(unsigned long long x) {
    x ^= x >> 32;
...
static unsigned long u32(unsigned long long *s) {
...
static unsigned long randint(unsigned long r, unsigned long long s[1]) {
...
static struct hf_op hf_gen(enum hf_type type, unsigned long long s[1]) {
    struct hf_op op;
    op.type = type;
    switch (op.type) {
    case HF16_NOT:
...
static void hf_genfunc(struct hf_op *ops, int n, unsigned long long s[1]) {
    for (int i = 0; i < n; i++) {
        do {
...
static int opdir(struct hf_op op) {
    switch (op.type) {
    case HF16_NOT:
    case HF16_XOR:
    case HF16_ADD:
...
static void hf_gensmart(struct hf_op *ops, int n, unsigned long long s[1]) {
    int dir = 0;
    for (int i = 0; i < n; i++) {
        int newdir;
        do {
...
static void hf_genxormul(struct hf_op *ops, int n, unsigned long long s[1]) {
...
#define AXS_SIZE  6
static void hf_genaxs(struct hf_op *ops, long i) {
    // clang-format off
    int shifts[] = {
        1 + (i /      1) % 15,
        1 + (i /     15) % 15,
...
static unsigned hf_apply(const struct hf_op *ops, int n, unsigned x) {
    for (int i = 0; i < n; i++) {
        // clang-format off
        switch (ops[i].type) {
...
static void hf_print(const struct hf_op *ops, int n, FILE *f) {
...
static void sbox_shuffle(unsigned long long s[1]) {
...
static void sbox_print(FILE *f) {
    for (long i = 0; i < 1L << 16; i++) {
        fprintf(f, "%04x%c", sbox[i], i % 16 == 15 ? '\n' : ' ');
    }
}

static double score(const struct hf_op *ops, int n) {
    long bins[32][32] = {{0}};
...
static int match(const struct hf_op *ops, int n, int types) {
    for (int i = 0; i < n; i++) {
