...
static uint64_t rand64(uint64_t s[4]) {
    uint64_t x = s[1] * 5;
...
static double estimate_bias32(const struct gene *g, uint64_t rng[4]) {
    long n            = 1L << QUALITY;
    long bins[32][32] = {{0}};
    for (long i = 0; i < n; i++) {
...
static double exact_bias32(const struct gene *g) {
...
static void gene_gen(struct gene *g, uint64_t rng[4]) {
...
static void gene_print(const struct gene *g, FILE *f) {
    fprintf(f, "[%2d %08lx %2d %08lx %2d]", g->s[0], (unsigned long)g->c[0], g->s[1],
            (unsigned long)g->c[1], g->s[2]);
}

static int small(uint64_t r) {
    static const int v[] = {-3, -2, -1, +1, +2, +3};
    return v[r % 6];
}

static void gene_mutate(struct gene *g, uint64_t rng[4]) {
...
static void gene_cross(struct gene *g, const struct gene *a, const struct gene *b,
                       uint64_t rng[4]) {
    uint64_t r = rand64(rng);
    *g         = *a;
    // clang-format off
...
static int gene_same(const struct gene *a, const struct gene *b) {
    // clang-format off
    return a->s[0] == b->s[0] &&
           a->s[1] == b->s[1] &&
           a->s[2] == b->s[2] &&
...
static void rng_init(void *p, size_t len) {
...
static void undup(struct gene *pool, uint64_t rng[4]) {
...
int main(void) {
    ...
    for (;;) {
        ...
        if (verbose) {
            for (int i = 0; i < POOL; i++) {
                if (!(pool[i].flags & FLAG_PRINTED) && pool[i].score < DONTCARE) {
